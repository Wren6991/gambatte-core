...
// RP2040: removed the disabled RAM segments, so here is some read-only
// all-ones to handle the OAM cases. This is flash-resident so we are still
// saving the full 16k of RAM
static const __attribute__((section(".flashdata.oam_blank"))) uint8_t oam_blank[256] = {
...
void Memory::loadState(SaveState const &state) {
	...
	lcd_.loadState(state, state.mem.oamDmaPos < oam_size ? oam_blank : ioamhram_);
